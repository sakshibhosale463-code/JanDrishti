...
public abstract partial class BaseProjectValidator<TModel> : AbstractValidator<TModel> where TModel : class
{
    #region Utilities
    ...
    {
        var entityDescriptor = ProjectMappingSchema.GetEntityDescriptor(typeof(TEntity));
    ...
    protected virtual void SetStringPropertiesMaxLength(ProjectEntityDescriptor entityDescriptor, params string[] filterPropertyNames)
    {
    ...
    /// </summary>
    /// <param name="entityDescriptor">Entity descriptor</param>
    protected virtual void SetDecimalMaxValue(ProjectEntityDescriptor entityDescriptor)
    {
        if (entityDescriptor is null)
            return;
