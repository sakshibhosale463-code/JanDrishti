...
namespace Project.Admin.Infrastructure.Installation;
...
public partial class InstallationLocalizationService : IInstallationLocalizationService
{
    ...
    protected readonly IProjectFileProvider _fileProvider;
    ...
    public string GetBrowserCulture()
    {
        ...
        return userLanguages.FirstOrDefault()?.Split(',').FirstOrDefault() ?? ProjectCommonDefaults.DefaultLanguageCulture;
    }
    ...
    public virtual InstallationLanguage GetCurrentLanguage()
    {
        ...
        var language = availableLanguages.FirstOrDefault();
        if (language != null)
            return language;
        ...
        foreach (var filePath in _fileProvider.EnumerateFiles(_fileProvider.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
        {
            ...
            foreach (XmlNode resNode in resources)
            {
                ...
                if (resNameAttribute == null)
                    throw new ProjectException("All installation resources must have an attribute Name=\"Value\".");
                var resourceName = resNameAttribute.Value.Trim();
                if (string.IsNullOrEmpty(resourceName))
                    throw new ProjectException("All installation resource attributes 'Name' must have a value.'");

                if (resValueNode == null)
                    throw new ProjectException("All installation resources must have an element \"Value\".");
