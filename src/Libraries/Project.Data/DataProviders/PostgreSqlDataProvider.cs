...
using Project.Core;
using Project.Data.DataProviders.LinqToDB;
using Project.Data.Mapping;
using Npgsql;

namespace Project.Data.DataProviders;

public partial class PostgreSqlDataProvider : BaseDataProvider, IProjectDataProvider
{
    #region Fields
    ...
    protected virtual string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
    {
        ...
        var descriptor = ProjectMappingSchema.GetEntityDescriptor(typeof(TEntity)) 
                         ?? throw new ProjectException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
        ...
        if (string.IsNullOrEmpty(columnName))
            throw new ProjectException("A table's primary key does not have an identity constraint");
    ...
    /// <summary>
    /// Performs bulk insert entities operation with identity false
    /// </summary>
    /// <typeparam name="TEntity">Entity type</typeparam>
    /// <param name="entities">Collection of Entities</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public override async Task BulkInsertWithoutIdentityEntitiesAsync<TEntity>(IEnumerable<TEntity> entities)
    {
        using var dataContext = CreateDataConnection(LinqToDbDataProvider);
        await dataContext.BulkCopyAsync(new BulkCopyOptions() { KeepIdentity = false }, entities);
    }
    ...
    public virtual string BuildConnectionString(IProjectConnectionStringInfo nopConnectionString)
    {
        ArgumentNullException.ThrowIfNull(nopConnectionString);

        if (nopConnectionString.IntegratedSecurity)
            throw new ProjectException("Data provider supports connection only with login and password");
