...
public abstract partial class BaseDataProvider
{
    ...
    protected virtual DataConnection CreateDataConnection(IDataProvider dataProvider)
    {
        ...
        var dataConnection = new DataConnection(dataProvider, CreateDbConnection(), ProjectMappingSchema.GetMappingSchema(ConfigurationName, LinqToDbDataProvider))
        {
    ...
    public virtual void InitializeDatabase()
    {
        ...
        var targetAssembly = typeof(ProjectDbStartup).Assembly;
    ...
    public virtual IQueryable<TEntity> GetTable<TEntity>() where TEntity : BaseEntity
    {
        var options = new DataOptions()
            ...
            .UseMappingSchema(ProjectMappingSchema.GetMappingSchema(ConfigurationName, LinqToDbDataProvider));
    ...
    /// The task result contains the inserted entity
    /// </returns>
    public virtual async Task<TEntity> InsertWithoutIdentityAsync<TEntity>(TEntity entity) where TEntity : BaseEntity
    {
        using var dataContext = CreateDataConnection();
    ...
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task BulkInsertWithoutIdentityEntitiesAsync<TEntity>(IEnumerable<TEntity> entities) where TEntity : BaseEntity
    {
        using var dataContext = CreateDataConnection(LinqToDbDataProvider);
