...
public partial class FluentMigratorMetadataReader : IMetadataReader
{
    ...
    public MappingAttribute[] GetAttributes(Type type)
    {
        ...
        {
            var entityDescriptor = ProjectMappingSchema.GetEntityDescriptor(type);

            if (entityDescriptor is null)
                return null;

            //get database and view mapping
            var viewName = NameCompatibilityManager.GetViewName(type);
            var databaseName = NameCompatibilityManager.GetDatabaseName(type);

            var tableAttribute = new TableAttribute() { Schema = entityDescriptor.SchemaName, Name = entityDescriptor.EntityName };

            if (!string.IsNullOrWhiteSpace(viewName))
            {
                tableAttribute.IsView = true;
                tableAttribute.Name = viewName;
                tableAttribute.IsColumnAttributeRequired = false;
            }

            if (!string.IsNullOrWhiteSpace(databaseName))
            {
                tableAttribute.Database = databaseName;
                tableAttribute.IsColumnAttributeRequired = false;
            }
