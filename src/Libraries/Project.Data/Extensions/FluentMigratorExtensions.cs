...
using Project.Data.Mapping.Builders;
...
public static class FluentMigratorExtensions
{
    ...
    public static ICreateTableColumnOptionOrForeignKeyCascadeOrWithColumnSyntax ForeignKey<TPrimary>(this ICreateTableColumnOptionOrWithColumnSyntax column, string primaryTableName = null, string primaryColumnName = null, Rule onDelete = Rule.None) where TPrimary : BaseEntity
    {
        ...
        if (string.IsNullOrEmpty(primaryColumnName))
            primaryColumnName = NameCompatibilityManager.GetColumnName(typeof(TPrimary), nameof(BaseEntity.Id));
    ...
    public static IAlterTableColumnOptionOrAddColumnOrAlterColumnOrForeignKeyCascadeSyntax ForeignKey<TPrimary>(this IAlterTableColumnOptionOrAddColumnOrAlterColumnSyntax column, string primaryTableName = null, string primaryColumnName = null, Rule onDelete = Rule.None) where TPrimary : BaseEntity
    {
        if (string.IsNullOrEmpty(primaryTableName))
            primaryTableName = NameCompatibilityManager.GetTableName(typeof(TPrimary));
    ...
    public static void RetrieveTableExpressions(this CreateTableExpressionBuilder builder, Type type)
    {
        ...
        var hasIdColumnMapping = true;
        var idColumnName = NameCompatibilityManager.GetColumnName(type, nameof(BaseEntity.Id));
        if (expression.Columns.Any(c => c.Name == idColumnName))
            hasIdColumnMapping = false;

        if (hasIdColumnMapping && (!expression.Columns.Any(c => c.IsPrimaryKey)))
        {
            var pk = new ColumnDefinition
            {
                Name = NameCompatibilityManager.GetColumnName(type,nameof(BaseEntity.Id)),
                Type = DbType.Int64,
                IsIdentity = true,
