...
public partial class EntityRepository<TEntity> : IRepository<TEntity> where TEntity : BaseEntity
{
    ...
    protected readonly IProjectDataProvider _dataProvider;
    ...
    public EntityRepository(IEventPublisher eventPublisher,
        IProjectDataProvider dataProvider,
    ...
    protected virtual async Task<IList<TEntity>> GetEntitiesAsync(Func<Task<IList<TEntity>>> getAllAsync, Func<IStaticCacheManager, CacheKey> getCacheKey)
    {
        ...
        var cacheKey = getCacheKey(_staticCacheManager)
                       ?? _staticCacheManager.PrepareKeyForDefaultCache(ProjectEntityCacheDefaults<TEntity>.AllCacheKey);
    ...
    /// <param name="getCacheKey">Function to get a cache key; pass null to don't cache; return null from this function to use the default key</param>
    /// <param name="includeDeleted">Whether to include deleted items (applies only to <see cref="Project.Core.Domain.Common.ISoftDeletedEntity"/> entities)</param>
    ...
    public virtual async Task<TEntity> GetByIdAsync(long? id, Func<ICacheKeyService, CacheKey> getCacheKey = null, bool includeDeleted = true, bool useShortTermCache = false)
    {
        ...
        var cacheKey = getCacheKey(cacheKeyService)
                       ?? cacheKeyService.PrepareKeyForDefaultCache(ProjectEntityCacheDefaults<TEntity>.ByIdCacheKey, id);
    ...
    /// The entity entry
    /// </returns>
    public virtual TEntity GetById(long? id, Func<ICacheKeyService, CacheKey> getCacheKey = null, bool includeDeleted = true)
    {
        if (!id.HasValue || id == 0)
            return null;
        ...
        TEntity getEntity()
        {
            return AddDeletedFilter(Table, includeDeleted).FirstOrDefault(entity => entity.Id == Convert.ToInt64(id));
        }
        ...
        //caching
        var cacheKey = getCacheKey(_staticCacheManager)
                       ?? _staticCacheManager.PrepareKeyForDefaultCache(ProjectEntityCacheDefaults<TEntity>.ByIdCacheKey, id);
    ...
    public virtual async Task<IList<TEntity>> GetByIdsAsync(IList<long> ids, Func<ICacheKeyService, CacheKey> getCacheKey = null, bool includeDeleted = true)
    {
        ...
        if (cacheKey == null && _usingDistributedCache)
            cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(ProjectEntityCacheDefaults<TEntity>.ByIdsCacheKey, ids);
        if (cacheKey != null)
        ...
        {
            await _staticCacheManager.SetAsync(_staticCacheManager.PrepareKeyForDefaultCache(ProjectEntityCacheDefaults<TEntity>.ByIdCacheKey, entity.Id), entity);
    ...
    /// <summary>
    /// Insert the entity entry without identity
    /// </summary>
    /// <param name="entity">Entity entry</param>
    /// <param name="publishEvent">Whether to publish event notification</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public async Task InsertWithoutIdentityAsync(TEntity entity, bool publishEvent = true)
    {
        ArgumentNullException.ThrowIfNull(entity);

        await _dataProvider.InsertWithoutIdentityAsync(entity);
    ...
    /// <param name="publishEvent">Whether to publish event notification</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public async Task InsertWithoutIdentityAsync(IList<TEntity> entities, bool publishEvent = true)
    {
        ArgumentNullException.ThrowIfNull(entities);

        using var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled);
        await _dataProvider.BulkInsertWithoutIdentityEntitiesAsync(entities);
    ...
    /// </returns>
    public virtual long Delete(Expression<Func<TEntity, bool>> predicate)
    {
        ArgumentNullException.ThrowIfNull(predicate);
    ...
    /// Retrive data in sp
    /// </summary>
    /// <param name="resetIdentity">list of pagination data</param>
    /// <returns>A task that represents the asynchronous operation</returns>
