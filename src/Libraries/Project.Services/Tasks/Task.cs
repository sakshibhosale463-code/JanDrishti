...
using Project.Core.Domain.Tasks;
using Project.Core.Infrastructure;
using Project.Services.Localization;
using Project.Services.Logging;
using Theading = System.Threading.Tasks;

namespace Project.Services.Tasks
{
    /// <summary>
    /// Task
    /// </summary>
    ...
    public partial class Task
    {
        ...
        /// </summary>
        private Theading.Task ExecuteTask()
        {
            var scheduleTaskService = EngineContext.Current.Resolve<IScheduleTaskService>();
            
            if (!Enabled)
                return Theading.Task.CompletedTask;
        ...
        public async System.Threading.Tasks.Task ExecuteAsync(bool throwException = false, bool ensureRunOncePerPeriod = true)
        {
            ...
            try
            {
                ...
                await locker.PerformActionWithLockAsync(ScheduleTask.Type, expiration, ExecuteTask);
            }
            catch (Exception exc)
            {
                var scheduleTaskService = EngineContext.Current.Resolve<IScheduleTaskService>();
                ...
                var commonConfig = EngineContext.Current.Resolve<AppSettings>().Get<CommonConfig>();
                var scheduleTaskUrl = $"{commonConfig.DomainUrl.TrimEnd('/')}/{NopTaskDefaults.ScheduleTaskPath}";
                ...
                var message = string.Format(await localizationService.GetResourceAsync("ScheduleTasks.Error"), ScheduleTask.Name,
                    exc.Message, ScheduleTask.Type, string.Empty, scheduleTaskUrl);
