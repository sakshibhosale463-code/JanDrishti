...
public partial class LocalizationService : ILocalizationService
{
    ...
    protected virtual async Task<IDictionary<string, string>> UpdateLocaleResourceAsync(IDictionary<string, string> resources, long? languageId = null, bool clearCache = true)
    {
        ...
        //clear cache
        if (clearCache)
            await _staticCacheManager.RemoveByPrefixAsync(ProjectEntityCacheDefaults<LocaleStringResource>.Prefix);

        return localResources.Where(item => !existsResources.Contains(item.Key, StringComparer.InvariantCultureIgnoreCase))
            .ToDictionary(p => p.Key, p => p.Value);
    }

    protected virtual IDictionary<string, string> UpdateLocaleResource(IDictionary<string, string> resources, long? languageId = null, bool clearCache = true)
    {
        var localResources = new Dictionary<string, string>(resources, StringComparer.InvariantCultureIgnoreCase);
        var keys = localResources.Keys.Select(key => key.ToLowerInvariant()).ToArray();
        ...
        //clear cache
        if (clearCache)
            _staticCacheManager.RemoveByPrefix(ProjectEntityCacheDefaults<LocaleStringResource>.Prefix);
    ...
    {
        var key = _staticCacheManager.PrepareKeyForDefaultCache(ProjectLocalizationDefaults.LocaleStringResourcesAllCacheKey, languageId);
        ...
        if (!loadPublicLocales.HasValue || allLocales != null)
        {
            ...
            await _staticCacheManager.RemoveAsync(ProjectLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey, languageId);
            await _staticCacheManager.RemoveAsync(ProjectLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey, languageId);
        ...
        key = _staticCacheManager.PrepareKeyForDefaultCache(loadPublicLocales.Value
                ? ProjectLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey
                : ProjectLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey,
            languageId);
        ...
        return await _staticCacheManager.GetAsync(key, async () =>
        {
            ...
            var query = from l in _lsrRepository.Table
                        ...
                        where l.LanguageId == languageId
                        select l;
            query = loadPublicLocales.Value ? query.Where(r => !r.ResourceName.StartsWith(ProjectLocalizationDefaults.AdminLocaleStringResourcesPrefix)) : query.Where(r => r.ResourceName.StartsWith(ProjectLocalizationDefaults.AdminLocaleStringResourcesPrefix));
        ...
        bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
    {
        ...
        if (_localizationSettings.LoadAllLocaleRecordsOnStartup)
        {
            ...
            var resources = await GetAllResourceValuesAsync(languageId, !resourceKey.StartsWith(ProjectLocalizationDefaults.AdminLocaleStringResourcesPrefix, StringComparison.InvariantCultureIgnoreCase));
        ...
        {
            //gradual loading
            var key = _staticCacheManager.PrepareKeyForDefaultCache(ProjectLocalizationDefaults.LocaleStringResourcesByNameCacheKey
    ...
    public virtual async Task<string> ExportResourcesToXmlAsync(Language language)
    {
        ...
        await xmlWriter.WriteAttributeStringAsync("SupportedVersion", ProjectVersion.CURRENT_VERSION);
    ...
    public virtual async Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, long? languageId = null) where TEnum : struct
    {
        ...
        //localized value
        var workingLanguage = await _workContext.GetWorkingLanguageAsync();
        var resourceName = $"{ProjectLocalizationDefaults.EnumLocaleStringResourcesPrefix}{typeof(TEnum)}.{enumValue}";
    ...
    public virtual async Task<string> GetLocalizedPermissionNameAsync(PermissionRecord permissionRecord, long? languageId = null)
    {
        ...
        //localized value
        var workingLanguage = await _workContext.GetWorkingLanguageAsync();
        var resourceName = $"{ProjectLocalizationDefaults.PermissionLocaleStringResourcesPrefix}{permissionRecord.SystemName}";
        var result = await GetResourceAsync(resourceName, languageId ?? workingLanguage.Id, false, string.Empty, true);
    ...
    /// <param name="resources">Resource name-value pairs</param>
    /// <param name="languageId">Language identifier; pass null to add the passed resources for all languages</param>
    public virtual void AddOrUpdateLocaleResource(IDictionary<string, string> resources, long? languageId = null)
    {
        //first update all previous locales with the passed names if they exist
    ...
    /// <param name="resourceNames">Resource names</param>
    /// <param name="languageId">Language identifier; pass null to delete the passed resources from all languages</param>
    public virtual void DeleteLocaleResources(IList<string> resourceNames, long? languageId = null)
    {
